<x-adminpanel::layouts.app :title="__('admin.profile.title')">
    <div class="mx-auto w-full max-w-3xl space-y-6">
        <x-adminpanel::components.page-header :title="__('admin.profile.title')" :description="__('admin.profile.description')" />
        ...
        <section class="card shadow-base-300/10 overflow-hidden shadow-md">
            <div class="card-body flex flex-col items-center gap-4 border-b p-6 text-center sm:flex-row sm:text-start">
                ...
                <div class="min-w-0">
                    <h2 class="line-clamp-2 break-words text-xl font-semibold">{{ auth()->user()?->name ?? __('admin.header.user') }}</h2>
            ...
            <dl class="divide-base-content/10 divide-y">
                ...
                <div class="grid gap-1 px-5 py-4 sm:grid-cols-3 sm:gap-4"><dt class="text-base-content/60 text-sm">{{ __('admin.navigation.roles') }}</dt><dd class="break-words font-medium sm:col-span-2">{{ auth()->user()?->getRoleNames()->join(', ') ?: '—' }}</dd></div>
                <div class="grid gap-1 px-5 py-4 sm:grid-cols-3 sm:gap-4"><dt class="text-base-content/60 text-sm">{{ __('admin.profile.status') }}</dt><dd class="font-medium sm:col-span-2">{{ auth()->user()?->is_active ? __('admin.common.active') : __('admin.common.inactive') }}</dd></div>
            </dl>
        </section>
        ...
        <form method="POST" action="{{ route('profile.photo.update') }}" enctype="multipart/form-data" class="card shadow-base-300/10 shadow-md">
            ...
            <div class="card-footer border-base-content/10 flex flex-wrap items-center justify-between gap-3 border-t p-4 sm:px-6">
                <p class="text-base-content/50 text-xs">{{ __('admin.profile.photo_hint') }}</p>
                <button type="submit" class="btn btn-primary"><span class="icon-[tabler--photo-check] size-5"></span>{{ __('admin.profile.save_photo') }}</button>
            </div>
        </form>
    </div>
